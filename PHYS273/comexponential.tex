...
\section{Complex Vectors}
	...
	Just as in standard trigonometry, the coordinate of $z$ is described as $\cos\theta + \sin\theta$, with the only difference being that the y-component is complex. Then, for any arbitrary $z = x + iy$, we can choose $A = x^2 + y^2$ and $\theta = \tan^{-1}(y/x)$ so that
	\begin{equation}
		z = x + iy = Ae^{i\theta}
	\end{equation}
